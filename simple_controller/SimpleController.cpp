...
SimpleController::SimpleController(const std::string &nodeName, const ControllerManager::StatePtr &stateEstimator)
        : ControllerManager(nodeName, stateEstimator) {
    initialized_ = false;
    rviz_sub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>("goal_pose", 10, std::bind(
            &SimpleController::rviz_callback, this, std::placeholders::_1)
    );

}

void SimpleController::rviz_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
...
tf2::Transform SimpleController::poseToTransform(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    tf2::Transform pose;
    double x, y, z;
...
void SimpleController::execute(const tf2::Transform &current_pose) {
    if(!initialized_)
        return;
