...
def generate_launch_description():
    ...
    pkg_create3_controller = get_package_share_directory('create3_controller')
    ...
    gazebo_controller_launch_file = PathJoinSubstitution(
        [pkg_create3_controller, 'launch', 'gazebo_controller.launch.py'])
    ...
    robot_description31 = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([robot_description_launch_file]),
        # launch_arguments={'namespace': 'ac31'}.items()
    )
    # robot_description32 = IncludeLaunchDescription(
    #     PythonLaunchDescriptionSource([robot_description_launch_file]),
    #     # launch_arguments={'namespace': 'ac32'}.items()
    # )
    gazebo_controller = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([gazebo_controller_launch_file])
    )
    spawn_robot31 = Node(
        package='gazebo_ros',
        executable='spawn_entity.py',
    ...
    # Create 3 nodes
    create31_nodes = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([create3_nodes_launch_file]),
        launch_arguments={'x': '0', 'y': '0', 'z': '0', 'yaw': '0', 'ns': 'ac31'}.items()
    )
    create32_nodes = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([create3_nodes_launch_file]),
        launch_arguments={'x': '1', 'y': '1', 'z': '0', 'yaw': '0', 'ns': 'ac32'}.items()
    )
    # RVIZ2
    ...
    ld.add_action(robot_description31)
    # ld.add_action(robot_description32)
    ld.add_action(spawn_robot31)
    ld.add_action(spawn_robot32)
    ld.add_action(spawn_dock)
    ld.add_action(dock_description)
    # Include Create 3 nodes
    ld.add_action(create31_nodes)
    ld.add_action(create32_nodes)
    # Rviz
    ld.add_action(rviz2)
    # controller 
